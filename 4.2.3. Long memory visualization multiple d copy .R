...
for (wich_d in 1:length(d_coef_vec)) {
  own_lambda_farima_matrix[wich_d,] = colMeans( (own_lambda_farima_list[[wich_d]]))
  own_lambda_fexp_matrix[wich_d,] = colMeans((own_lambda_fexp_list[[wich_d]]))
  r_lambda_matrix[wich_d,] = colMeans((r_lambda_list[[wich_d]]))
}
...
matplot(t(own_lambda_fexp_matrix), ylim=c(0.9,1.1),type = "l", col = row_colors, lty = 1,lwd = 1.5, xlab = "T", ylab = TeX("$\\bar{\\hat{\\lambda}}_{MLE}$"), cex.lab = lab_size, cex.axis = axis_text, labels = FALSE)
mtext(TeX(main), side = 3, line = -1, outer = TRUE, cex=tite_size, font = 2)
axis(1, at=1:8, labels = names)
axis(2)
legend("topright", inset = c(inset_num, 0), legend = c(0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45),
       col = row_colors, lty = 1, title = "d")
...
matplot(t(r_lambda_matrix), ylim=c(0.9,1.1),type = "l", col = row_colors, lty = 1,lwd = 1.5, xlab = "T", ylab = TeX("$bar{(\\hat{\\lambda}}_{MLE}$"), cex.lab = lab_size, cex.axis = axis_text, labels = FALSE)
mtext(TeX(main), side = 3, line = -1, outer = TRUE, cex=tite_size, font = 2)
axis(1, at=1:8, labels = names)
axis(2)
legend("topright", inset = c(inset_num, 0), legend = c(0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45),
