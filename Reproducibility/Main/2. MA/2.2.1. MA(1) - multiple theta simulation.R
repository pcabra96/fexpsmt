...
path = paste0("~/Documents/2. UNIGE/2023-1 Master Thesis/fexpsmt/Reproducibility/Main/2. ",PROCESS,"/")
...
# Visualization
rownames(average_time_own) = ma_coef_vec
colnames(average_time_own) = POWER
rownames(average_time_r) = ma_coef_vec
...
rownames(MSE_own) = ma_coef_vec
colnames(MSE_own) = POWER
rownames(MSE_r) = ma_coef_vec
colnames(MSE_r) = POWER

saveRDS(MSE_own, file = paste0(path,"theta_1_own.RData"))
saveRDS(MSE_r, file = paste0(path,"theta_1_r.RData"))
...
rownames(prop_non_rejection_own) = ma_coef_vec
colnames(prop_non_rejection_own) = POWER
rownames(prop_non_rejection_r) = ma_coef_vec
