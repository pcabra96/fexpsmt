...
POWER = 7:14
N_SIM = 1000 # EXPECTED TIME FOR 1000 simulations is
K = 2
...
for (sim in 1:N_SIM) {
    for (j in 1:length(POWER)){
      ...
      d_coef = runif(n = 1, min = 0.05, max = 0.45)
      ...
      # FITTING
      ##################################
      a = fracdiff(x = y_ar,nar = 1)
      fit_ar_ar[sim,j] = a$ar
      fit_d_ar[sim,j] = a$d

      a = fracdiff(x = y_exp,nar = 1)
      fit_ar_exp[sim,j] = a$ar
      fit_d_exp[sim,j] = a$d

      a = fit.fexp(y_ar,p = K, d = 1)
      fit_c_k_ar[[j]][sim,] = a$c_k
      fit_d_ar_2[sim,j] = a$d

      a = fit.fexp(y_exp,p = K, d = 1)
      fit_c_k_exp[[j]][sim,] = a$c_k
      fit_d_exp_2[sim,j] = a$d
...
saveRDS(true_d, file = paste0(path,"true_d.RData"))
...
saveRDS(fit_d_ar_2, file = paste0(path,"fit_d_ar_2.RData"))
saveRDS(fit_d_exp_2, file = paste0(path,"fit_d_exp_2.RData"))

################################################################################
# 5.4. TIME DOMAIN PARAMETER AR
################################################################################
...
saveRDS(fit_d_ar, file = paste0(path,"fit_d_ar.RData"))
saveRDS(fit_d_exp, file = paste0(path,"fit_d_exp.RData"))
