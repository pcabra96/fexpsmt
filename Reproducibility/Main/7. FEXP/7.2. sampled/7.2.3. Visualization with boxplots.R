...
PROCESS = "FEXP"
SUBPROCESS = "sampled"
path = paste0("~/Documents/2. UNIGE/2023-1 Master Thesis/fexpsmt/Reproducibility/Main/7. ",PROCESS,"/7.1. ",SUBPROCESS,"/")
...
graph_1 = TeX("$fepxmst_{FARIMA(1,d,0)}$")
graph_2 = TeX("$fepxmst_{FEXP(6,d)}$")
...
################################################################################
# 5.4. MSE COEFFICIENTS FOR XI COEFFICIENT
################################################################################
...
# LOAD THE DATA
true_d <- readRDS(file = paste0(path, "true_d.RData"))
fit_d_ar <- readRDS(file = paste0(path, "fit_d_ar.RData"))
fit_d_exp <- readRDS(file = paste0(path, "fit_d_exp.RData"))
...
# LOAD THE DATA
true_d <- readRDS(file = paste0(path, "true_d.RData"))
fit_d_ar_2 <- readRDS(file = paste0(path, "fit_d_ar_2.RData"))
fit_d_exp_2 <- readRDS(file = paste0(path, "fit_d_exp_2.RData"))
...
# LIMITS
lim_inf = min(ar_lambda,exp_lambda)
lim_sup = max(ar_lambda,exp_lambda)
...
# LIMITS
lim_inf = min(ar_exp_1,exp_exp_1)
lim_sup = max(ar_exp_1,exp_exp_1)
...
# MSE
y_1 = colMeans((ar_exp_1>=0.05))
y_2 = colMeans((exp_exp_1>=0.05))

# STANDARD DEVIATION OF SQUARED ERROR
sd_own <- apply((ar_exp_1>=0.05), 2, sd)
sd_r <- apply((exp_exp_1>=0.05), 2, sd)
