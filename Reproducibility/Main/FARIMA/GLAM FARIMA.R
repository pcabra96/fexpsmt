...
for (sim in 1:N_SIMULATIONS) {
    ...
    X = as.matrix(cbind(1,log(2*sin(w/2)),cos(w),cos(2*w)))
    ...
    phi_hat_fexpmst = (beta_2_fexpmst^2+beta_3_fexpmst)/(2*beta_2_fexpmst)
    ...
    phi_hat_fracdiff = (beta_2_fracdiff^2+beta_3_fracdiff)/(2*beta_2_fracdiff)
    ...
    phi_hat_arfima = (beta_2_arfima^2+beta_3_arfima)/(2*beta_2_arfima)
